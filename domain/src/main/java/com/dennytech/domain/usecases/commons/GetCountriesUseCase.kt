...
) : BaseSuspendUseCase<Unit, List<CountryDomainModel>>(dispatcher) {

    override suspend fun run(param: Unit?): List<CountryDomainModel> {
        return runBlocking { locationRepository.getCountries() }.first()
    }
}