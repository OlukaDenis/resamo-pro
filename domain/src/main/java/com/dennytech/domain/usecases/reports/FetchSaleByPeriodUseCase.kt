...
class FetchSaleByPeriodUseCase @Inject constructor(
    private val dispatcher: AppDispatcher,
    private val utilRepository: UtilRepository,
    private val reportsRepository: ReportsRepository
) : BaseFlowUseCase<Unit, Resource<List<SaleReportDomainModel>>>(dispatcher) {
    override fun run(param: Unit?): Flow<Resource<List<SaleReportDomainModel>>> = flow{
        emit(Resource.Loading)

        try {
            val response = runBlocking { reportsRepository.fetchSalesByPeriod() }
